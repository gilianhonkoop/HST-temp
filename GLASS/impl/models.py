...
def normalizedEdgeWeight(edge_index, edge_weight, n_node: int, aggr: str):
    if edge_weight.dim() == 1:
        adj_weight = edge_weight
    elif edge_weight.size(-1) == 1:
        adj_weight = edge_weight.reshape(-1)
    else:
        adj_weight = torch.ones(edge_index.size(1), device=edge_index.device)

    adj_weight = adj_weight.to(torch.float)
    ...
    if aggr == "mean":
        deg = 1.0 / deg
        return deg[edge_index[0]] * adj_weight
    elif aggr == "sum":
        return adj_weight
    elif aggr == "gcn":
        deg = torch.pow(deg, -0.5)
        return deg[edge_index[0]] * adj_weight * deg[edge_index[1]]
    else:
...
def buildAdj(edge_index, edge_weight, n_node: int, aggr: str):
    '''
        ...
        '''
    norm_weight = normalizedEdgeWeight(edge_index, edge_weight, n_node, aggr)
...
class GLASSConv(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if edge_feature_dim and edge_feature_dim > 0:
            self.edge_encoder = nn.Linear(edge_feature_dim, out_channels)
    ...
    def forward(self, x_, edge_index, edge_weight, mask):
        if self.adj.shape[0] == 0:
            ...
            self.norm_edge_weight = normalizedEdgeWeight(edge_index,
                                                         edge_weight,
                                                         n_node,
                                                         self.aggr)
        ...
        if self.edge_encoder is None:
            ...
            msg = x[edge_index[1]] + self.edge_encoder(edge_feature.to(x.dtype))
            msg = msg * self.norm_edge_weight.reshape(-1, 1).to(x.dtype)
            out = torch.zeros_like(x)
            x = out.index_add(0, edge_index[0], msg)
        x = self.gn(x)
