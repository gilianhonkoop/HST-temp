...
def get_minority_class(labels):
    ...
    classes, counts = torch.unique(labels_cpu, return_counts=True)
    if len(classes) == 0:
        return 0
    return int(classes[torch.argmin(counts)].item())

def binary_threshold_predictions(logits, labels, threshold: float = 0.5, target_class=None):
    if target_class is None:
        target_class = get_minority_class(labels)

    probs = F.softmax(logits, dim=1)
    target_class = int(target_class)
    other_class = 1 - target_class
    pred = torch.full_like(labels, fill_value=other_class)
    pred[probs[:, target_class] > threshold] = target_class
    return pred

def calc_f1(logits, labels, avg_type='macro',  multilabel_binarizer=None, threshold: float = 0.5):
    '''
    ...
    if multilabel_binarizer is not None: #multi-label prediction
        # perform a sigmoid on each logit separately & use threshold to make prediction
        probs = torch.sigmoid(logits)
        thresh = torch.tensor([threshold]).to(probs.device)
        pred = (probs > thresh)
        score = f1_score(labels.cpu().detach(), pred.cpu().detach(), average=avg_type)
    else: # multi-class or binary prediction
        if logits.dim() == 2 and logits.shape[1] == 2:
            pred = binary_threshold_predictions(logits, labels, threshold)
        else:
            pred = torch.argmax(logits, dim=-1) #get predictions by finding the indices with max logits
        score = f1_score(labels.cpu().detach(), pred.cpu().detach(), average=avg_type)
    return torch.tensor([score])

def calc_accuracy(logits, labels,  multilabel_binarizer=None, threshold: float = 0.5):
    '''
...
def calc_minor_f1(logits, labels, multilabel_binarizer=None, threshold: float = 0.5):
    if multilabel_binarizer is not None:
        probs = torch.sigmoid(logits)
        pred = (probs > threshold).int().cpu().numpy()
        true = labels.cpu().numpy()
        support = true.sum(axis=0)
        minor_idx = int(support.argmin()) if support.size > 0 else 0
        score = f1_score(true[:, minor_idx], pred[:, minor_idx])
        return torch.tensor([score])

    if logits.dim() == 2 and logits.shape[1] == 2:
        preds = binary_threshold_predictions(logits, labels, threshold)
    else:
    ...
    minor_class = get_minority_class(labels_cpu)
    score = f1_score(labels_cpu.numpy(), preds_cpu.numpy(), labels=[minor_class], average='macro')
    return torch.tensor([score])

def calc_auprc(logits, labels, multilabel_binarizer=None):
    if multilabel_binarizer is not None:
    ...
    if len(torch.unique(labels)) == 2:
        y_true = labels.cpu().numpy()
        y_score = F.softmax(logits, dim=1).cpu().numpy()[:, 1]
        return average_precision_score(y_true, y_score)

    one_hot_labels = one_hot(labels, num_classes=logits.shape[1]).cpu().numpy()
    y_score = F.softmax(logits, dim=1).cpu().numpy()
    return average_precision_score(one_hot_labels, y_score, average='macro')

def calc_minor_pr_auc(logits, labels, multilabel_binarizer=None):
    """
    Calculate PR-AUC for the minority (illicit) class.
    For multilabel, use the class with the smallest support.
    For binary/multiclass, use the class with the smallest support in labels.
    """
    if multilabel_binarizer is not None:
        y_true = labels.cpu().numpy()
        y_score = torch.sigmoid(logits).cpu().numpy()
        ...
        minor_idx = int(support.argmin()) if support.size > 0 else 0
        return average_precision_score(y_true[:, minor_idx], y_score[:, minor_idx])

    labels_cpu = labels.cpu()
    classes, counts = torch.unique(labels_cpu, return_counts=True)
    if len(classes) == 0:
        return float("nan")
    minor_class = int(classes[torch.argmin(counts)].item())

    if logits.dim() == 2 and logits.shape[1] >= 2:
        probs = F.softmax(logits, dim=1).cpu().numpy()
        y_score = probs[:, minor_class]
    else:
        # Fallback for single-logit binary case
        y_score = torch.sigmoid(logits).view(-1).cpu().numpy()
        if minor_class == 0:
            y_score = 1.0 - y_score

    y_true = (labels_cpu.numpy() == minor_class).astype(int)
    return average_precision_score(y_true, y_score)
...
def get_border_nodes(graph, subgraph):
    ...
    border_nodes = subgraph_nodes[border_edge_exists]
