    ...
    for anchor_patch in anchor_patch_ids:
        ...
        if curr_anchor_patch.shape[0] == 0:
            ...
            if not inside: 
                ...
                if len(in_border_nodes) == 0 or len(all_valid_nodes) == 0:
                    walk_inside = True
            ...
            for w in range(hparams['n_triangular_walks']):
                walk = triangular_random_walk(hparams, networkx_graph, anchor_patch_subgraph, hparams['random_walk_len'], in_border_nodes, all_valid_nodes, inside=walk_inside)
