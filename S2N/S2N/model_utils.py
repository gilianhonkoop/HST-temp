...
class MLP(nn.Module):
    ...
    def __init__(self, num_layers, in_channels, hidden_channels, out_channels, activation,
                 use_bn=False, use_gn=False, dropout=0.0, activate_last=False, batch=None):
        super().__init__()
        ...
        for i, (layer_name, num_layers) in enumerate(zip(layer_name_list, num_layers_list)):
            ...
            self.add_module(
                ...
                GraphEncoder(layer_name, num_layers, _in_channels, hidden_channels, _out_channels,
                             ...
                             _activate_last, force_lin_x_0, edge_attr_mode, **kwargs))
...
class GraphEncoder(nn.Module):
    ...
    def prepare_edge_attr(self, conv, edge_attr):
        if edge_attr is None:
            return None

        mode = self.edge_attr_mode
        if mode in {None, "auto"}:
            mode = "weight" if isinstance(conv, (GCNConv, MyGCN2Conv, MyFAConv)) else "full"
        if mode in {"none", "None", "drop", False}:
            return None
        ...
        if mode == "weight":
            return edge_attr[:, 0].contiguous()
        if mode == "features":
            edge_attr = edge_attr[:, 1:] if edge_attr.size(1) > 1 else edge_attr
            return edge_attr[:, 0].contiguous() if isinstance(conv, (GCNConv, MyGCN2Conv, MyFAConv)) else edge_attr
        if mode == "full":
            return edge_attr

        raise ValueError(f"Wrong edge_attr_mode: {self.edge_attr_mode}")
    ...
    def forward(self, x, edge_index, edge_attr=None, batch=None, **kwargs):
        ...
        for i, conv in enumerate(self.convs):
            ...
            _edge_attr = self.prepare_edge_attr(conv, _edge_attr)
