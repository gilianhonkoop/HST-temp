...
from data_sub import (
    HPONeuro, PPIBP, HPOMetab, EMUser, Density, Component, Coreness, CutRatio,
    SubgraphDataset
)
from aml import AMLHI, SAMLD
...
class SubgraphDataModule(LightningDataModule):
    # Process-local cache to reuse prepared data across repeated datamodule
    # instantiations (e.g., sequential Optuna trials with n_jobs=1).
    _CROSS_TRIAL_CACHE: Dict[Tuple[Tuple[str, str], ...], Tuple[Any, Any, Any, Any]] = {}
    ...
    def __init__(self,
                 ...
                 s2n_original_edge_attr_mode: Optional[str] = None,
                 s2n_original_edge_attr_aggr: str = "mean",
                 s2n_original_edge_attr_normalize: Optional[str] = None,
                 s2n_original_edge_attr_norm_eps: float = 1e-6,
                 s2n_original_edge_attr_fill_value: float = 0.0,
                 s2n_transform=None,
                 ...
                 reuse_data_across_trials: bool = True,
                 num_workers=0,
                 verbose=2,
                 prepare_data=False,
                 log_func=None,
    ...
    def _build_cross_trial_cache_key(self) -> Tuple[Tuple[str, str], ...]:
        # Exclude runtime-only knobs that do not change actual graph data.
        excluded = {
            "batch_size", "eval_batch_size", "num_workers", "verbose",
            "prepare_data", "log_func", "logger", "reuse_data_across_trials",
        }
        items = []
    ...
    def s2n_path(self) -> str:
        ...
        if self.h.dataset_name in ["WLKSRandomTree", "AMLHI"]:
            key_dir = getattr(self.dataset, "key_dir", None)
            if key_dir:
                return os.path.join(key_dir, folder_name)
            # Fallback for datasets without key_dir
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        # Lightning may call setup() multiple times (e.g., for fit/test).
        # Our setup builds all splits at once, so reuse in-memory data.
        if self._is_setup_done and self.dataset is not None \
            and self.train_data is not None and self.val_data is not None and self.test_data is not None:
            return

        use_cross_trial_cache = bool(getattr(self.h, "reuse_data_across_trials", True))
        cache_key = self._build_cross_trial_cache_key() if use_cross_trial_cache else None
        if use_cross_trial_cache and cache_key in self._CROSS_TRIAL_CACHE:
            cached_dataset, cached_train, cached_val, cached_test = self._CROSS_TRIAL_CACHE[cache_key]
            self.dataset = cached_dataset
            self.train_data = cached_train
            self.val_data = cached_val
            self.test_data = cached_test
        ...
        if self.h.use_s2n:
            s2n_class = SubgraphToNodePlusCoarsening if self.h.use_coarsening else SubgraphToNode
            s2n_kwargs = {
                'path': self.s2n_path,
                'matrix_type': self.h.s2n_target_matrix,
                'use_sub_edge_index': self.h.s2n_use_sub_edge_index,
                'target_matrix': self.h.s2n_target_matrix,
                'add_sub_x_wl': self.h.s2n_add_sub_x_wl,
                'set_sub_x_weight': self.h.s2n_set_sub_x_weight,
                'is_weighted': self.h.s2n_is_weighted,
                'global_data': self.dataset.global_data,
                'subgraph_data_list': self.dataset.subgraph_data_list,
                'splits': self.dataset.splits,
                'name': self.dataset.name,
                'edge_aggr': self.h.s2n_edge_aggr,
                'original_edge_attr_mode': self.h.s2n_original_edge_attr_mode,
                'original_edge_attr_aggr': self.h.s2n_original_edge_attr_aggr,
                'original_edge_attr_normalize': self.h.s2n_original_edge_attr_normalize,
                'original_edge_attr_norm_eps': self.h.s2n_original_edge_attr_norm_eps,
                'original_edge_attr_fill_value': self.h.s2n_original_edge_attr_fill_value,
                'num_workers': self.h.num_workers,
                'verbose': self.h.verbose,
            }
            s2n_transform = s2n_class(**s2n_kwargs)
            # s2n_transform = s2n_class(path=self.s2n_path,
            #                           # matrix_type='adj',
            #                           matrix_type=self.h.s2n_target_matrix,
            #                           use_sub_edge_index=self.h.s2n_use_sub_edge_index,
            #                           target_matrix=self.h.s2n_target_matrix,
            ...
            if self.h.use_coarsening:
                s2n_transform.coarsening(ratio=self.h.coarsening_ratio,
                                         method=self.h.coarsening_method)
            data_list = s2n_transform.node_task_data_splits(
            ...
            if self.h.s2n_add_sub_x_wl:
                data_list = s2n_transform.node_task_add_sub_x_wl(
        ...
        if use_cross_trial_cache and cache_key is not None:
            self._CROSS_TRIAL_CACHE[cache_key] = (
                self.dataset,
                self.train_data,
                self.val_data,
                self.test_data,
