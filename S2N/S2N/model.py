...
class GraphNeuralModel(LightningModule):
    ...
    def __init__(self,
        ...
        if self.h.use_s2n:
            ...
            if self.h.sub_node_encoder_name == "DeepSets":
                if self.h.sub_node_num_layers == 0:
                    encoder = nn.Linear(self.node_emb.num_channels, self.h.hidden_channels)
                    decoder = MyIdentity()
        ...
        encoder_layer_kwargs = dict(self.h.layer_kwargs)
        ...
        if given_datamodule.h.s2n_is_weighted:
            if self.h.encoder_layer_name in ["GATConv", "GATv2Conv"]:
                edge_attr = getattr(given_datamodule.train_data, "edge_attr", None)
                edge_dim = 1
                if isinstance(edge_attr, torch.Tensor):
                    edge_dim = edge_attr.size(1) if edge_attr.dim() > 1 else 1
                    edge_attr_mode = encoder_layer_kwargs.get("edge_attr_mode", "auto")
                    if edge_attr_mode == "features" and edge_dim > 1:
                        edge_dim -= 1
                    elif edge_attr_mode in {"weight", "none", "drop"}:
                        edge_dim = 1
                encoder_layer_kwargs["edge_dim"] = edge_dim

        if self.h.encoder_layer_name == "LINKX":
            encoder_layer_kwargs.pop("edge_attr_mode", None)
            self.encoder = InductiveLINKX(
                ...
                **encoder_layer_kwargs,  # num_edge_layers, num_node_layers
            )
        else:
        ...
        if not self.h.is_multi_labels:
            if self.h.loss_weight is not None:
                weight = torch.tensor(self.h.loss_weight)
        ...
        self.evaluator = Evaluator(self.h.metrics, self.h.is_multi_labels, self.given_datamodule.num_classes)
        self.hparams.train_val_threshold = train_val_threshold
        self.hparams.test_threshold = test_threshold
        self._warned_invalid_edge_index = False
        self._train_outputs = []
        self._valid_outputs = []
        self._test_outputs = []
    ...
    @staticmethod
    def _to_epoch_record(step_out: Dict[str, Any]) -> Dict[str, Any]:
        return {
            "loss_sum": step_out["loss_sum"],
            "size": step_out["size"],
            "logits": step_out["logits"].detach(),
            "ys": step_out["ys"].detach(),
        }
    ...
    def forward(self,
        ...
        if isinstance(edge_index, torch.Tensor) and edge_index.dim() == 2 and edge_index.size(0) == 2 and x is not None:
            num_nodes = x.size(0)
            valid = (edge_index[0] >= 0) & (edge_index[0] < num_nodes) & (edge_index[1] >= 0) & (edge_index[1] < num_nodes)
            if valid.numel() > 0 and not bool(valid.all()):
                if not self._warned_invalid_edge_index:
                    num_invalid = int((~valid).sum().item())
                    log.warning(f"Filtered {num_invalid} invalid edges with out-of-range node indices.")
                    self._warned_invalid_edge_index = True
                edge_index = edge_index[:, valid]
                if isinstance(edge_attr, torch.Tensor) and edge_attr.size(0) == valid.size(0):
    ...
    def training_step(self, batch: Data, batch_idx: int):
        out = self.step(batch=batch, batch_idx=batch_idx)
        self._train_outputs.append(self._to_epoch_record(out))
        return out

    def validation_step(self, batch: Data, batch_idx: int):
        out = self.step(batch=batch, batch_idx=batch_idx)
        self._valid_outputs.append(self._to_epoch_record(out))
        return out

    def test_step(self, batch: Data, batch_idx: int):
        out = self.step(batch=batch, batch_idx=batch_idx)
        self._test_outputs.append(self._to_epoch_record(out))
        return out
    ...
    def epoch_end(self, prefix, output_as_dict):
        ...
        alias_prefix = "val" if prefix == "valid" else None
        if alias_prefix is not None:
            self.log(f"{alias_prefix}/loss", sum(loss_sum) / sum(size), prog_bar=False)

        logits = torch.cat(logits)  # [*, C]
        ys = torch.cat(ys)  # [*] or [*, C]

        threshold = self.hparams.test_threshold if prefix == "test" else self.hparams.train_val_threshold
        
        
        evaluated = self.evaluator(logits, ys, threshold=threshold)
        for metric, value in evaluated.items():
            if "class" in metric:
                # Assuming class names are 'illicit' and 'licit'
                class_name = "illicit" if "0" in metric else "licit"
                metric_name = metric.replace(f"_class_{metric.split('_')[-1]}", "")
                self.log(f"{prefix}/{class_name}_{metric_name}", value, prog_bar=True)
                if alias_prefix is not None:
                    self.log(f"{alias_prefix}/{class_name}_{metric_name}", value, prog_bar=True)
            else:
                self.log(f"{prefix}/{metric}", value, prog_bar=True)
                if alias_prefix is not None:
                    self.log(f"{alias_prefix}/{metric}", value, prog_bar=True)
            if prefix == "test" and self.h.hp_metric == metric:
                self.logger.log_metrics({"hp_metric": float(value)})

    def on_train_epoch_end(self) -> None:
        if len(self._train_outputs) == 0:
            return
        self.epoch_end("train", ld_to_dl(self._train_outputs))
        self._train_outputs.clear()

    def on_validation_epoch_end(self) -> None:
        if len(self._valid_outputs) == 0:
            return
        self.epoch_end("valid", ld_to_dl(self._valid_outputs))
        if not getattr(self, "_keep_outputs", False):
            self._valid_outputs.clear()

    def on_test_epoch_end(self) -> None:
        if len(self._test_outputs) == 0:
            return
        self.epoch_end("test", ld_to_dl(self._test_outputs))
        if not getattr(self, "_keep_outputs", False):
