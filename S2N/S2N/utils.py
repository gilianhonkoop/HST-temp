...
def func_normalize(x: Tensor, norm_type: str, **kwargs):
    if norm_type == "linear":
        x_min, x_max = kwargs.get("x_min", x.min()), kwargs.get("x_max", x.max())
        x = (x - x_min) / (x_max - x_min)
    elif norm_type == "standardize":
        x_mean, x_std = kwargs.get("x_mean", x.mean()), kwargs.get("x_std", x.std())
        x = (x - x_mean) / x_std
    elif norm_type == "trunc_thres":
        thres = kwargs.get("thres", 1.0)
        x[x > thres] = thres
    elif norm_type == "max":
        x = x / x.max()
    elif norm_type.startswith("standardize_then_trunc_thres_max"):
        x_mean, x_std = kwargs.get("x_mean", x.mean()), kwargs.get("x_std", x.std())
        x = (x - x_mean) / x_std
        thres = kwargs.get("thres", 1.0)
        x[x > thres] = thres
        ...
        raise ValueError(f"Wrong norm_type: {norm_type}")
    return x, {"x_mean": x.mean(), "x_std": x.std(), "x_min": x.min(), "x_max": x.max()}
